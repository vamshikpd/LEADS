...
import plm.ws.mtom.photo.ParoleeReleaseDate;
import plm.ws.mtom.photo.VehicleYear;
import plm.ws.mtom.photo.DataAttachment;
import plm.ws.mtom.photo.Error;
import plm.ws.mtom.photo.InternalParoleePhotoRequest;
import plm.ws.mtom.photo.InternalParoleePhotoResponse;
import plm.ws.mtom.photo.ObjectFactory;

import com.plm.dataretrival.DataException;
import com.plm.dataretrival.ParoleePhotoProvider;
 ...
 */
@Endpoint
public class ParoleePhotoEndpoint {

    private static final ObjectFactory objectFactory = new ObjectFactory();
	private static final Logger logger = Logger.getLogger(ParoleePhotoEndpoint.class);
...
//        this.objectFactory =
    ...
    @PayloadRoot(localPart = "InternalParoleePhotoRequest", namespace = "http://www.plm/ws/mtom/photo")
	@ResponsePayload
    public JAXBElement<InternalParoleePhotoResponse> getParoleePhoto(@RequestPayload JAXBElement<InternalParoleePhotoRequest> reqElement){

		MDC.put("id", System.nanoTime());
	...
	private JAXBElement<InternalParoleePhotoResponse> generateResponse(JAXBElement<InternalParoleePhotoRequest> reqElement) {
		...
		ParoleePhotoProvider paroleePhotoProvider;
		InternalParoleePhotoResponse pdResponse = new InternalParoleePhotoResponse();
		String errorCode;
		...
		InternalParoleePhotoRequest pdReq = reqElement.getValue();
		...
		try{
			paroleePhotoProvider = new ParoleePhotoProvider(scInfo.getUsername());
			fileName = paroleePhotoProvider.getParoleePhoto(scInfo);
		}catch(DataException e){
			logger.error(scInfo.toString());
		...
		return objectFactory.createInternalParoleePhotoResponse(pdResponse);
	}
	 ...
	 * @return
	 */
	private JAXBElement<InternalParoleePhotoResponse> generateErrorResponse(String errorCode){
		InternalParoleePhotoResponse pdResponse = new InternalParoleePhotoResponse();
		Error err = new Error();
		err.setErrorCode(errorCode);
		err.setErrorMessage(errorMap.get(errorCode));
     ...
     */
    private SearchCriteriaInfo convertToSearchCriteria(InternalParoleePhotoRequest pdRequest) throws ParseException {
	 ...
	 * @param pdRequest - Parolee Photo Request
	 * @throws ParseException 
	 ...
	 */
	private String validateRequest(InternalParoleePhotoRequest pdRequest) {
     ...
     * @param req
     */
    private void printRequest(JAXBElement<InternalParoleePhotoRequest> req) {
		if( req != null ) {
			InternalParoleePhotoRequest pdr = req.getValue();
			if(pdr!=null){
    ...
    /**
     * 
     * @param tmpCounties County list
     * @return
     */
	public String printCounties(List<String> tmpCounties){
