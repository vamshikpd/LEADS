...
public class EndecaQueryExecuter implements IQueryExecuter {
	...
	private int MAX_IN_PARAMETERS_LIMIT = 20;
	private boolean paginationEnabled = true;
	private long lTotalRecords = 0;
	private String resultFileOutputPath = null;
	...
	public void init() {
		lTotalRecords = 0;
		resultFileOutputPath = null;
		resultFilename = null;

		PAGE_MAX_RECORDS = ConfigDataProvider.getInstance().getWSConfig().getRecordsPerPage();
		MAX_IN_PARAMETERS_LIMIT = ConfigDataProvider.getInstance().getWSConfig().getSqlInParameterLimit();
	}
	...
	/**
	 * @return the resultFileOutputPath
	 */
	...
	/**
	 * @param resultFileOutputPath
	 *            the resultFileOutputPath to set
	 */
	...
	protected String formatRecord(PropertyMap pm) {
		StringBuffer formatedOutput = new StringBuffer(pm.toString());
	...
	public String[] getCdcNumbersForSearchCriteria(SearchCriteriaInfo searchCriteria) throws DataException{
		int iInParameterCounter = 0;
		int iCdcNumArrayIndex = 0;
		String aCdcNumbers[] = null;
		StringBuffer sbCdcNumbers = new StringBuffer();
		Collection<PropertyContainer> collData = fetchCdcNumber(searchCriteria, 0);
		long totalRecords = getTotalRecords();
		logger.warn(searchCriteria.getUsername() + ": totalRecords: " + totalRecords);

		int sbSize = (int) (totalRecords / MAX_IN_PARAMETERS_LIMIT);
		if (totalRecords % MAX_IN_PARAMETERS_LIMIT > 0) {
			sbSize++;
		}
		aCdcNumbers = new String[sbSize];
		if (totalRecords <= PAGE_MAX_RECORDS) {
			// Create the COMMA separated CDC Number according to the Max In Parameter Limit.
			if (collData != null) {
				...
				while (iteERecs.hasNext()) {
					...
					sbCdcNumbers.append("," + (String) pm.get("CDC Number"));
					iInParameterCounter++;
					if (iInParameterCounter >= MAX_IN_PARAMETERS_LIMIT) {
						if (sbCdcNumbers.length() > 0) {
							// Remove first comma (,)
							aCdcNumbers[iCdcNumArrayIndex] = sbCdcNumbers.toString().substring(1);
						}
						iCdcNumArrayIndex++;
						iInParameterCounter = 0;
						sbCdcNumbers.replace(0, sbCdcNumbers.length(), "");
					}
				}
				// If the CDC Numbers are less then the MAX_IN_PARAMETER_LIMIT then add here.
				if (iInParameterCounter > 0) {
					aCdcNumbers[iCdcNumArrayIndex] = sbCdcNumbers.toString().substring(1);
				}
			}
		} else {
			...
			while (lReadCounter < totalRecords) {
				...
				lReadCounter = lReadCounter + collData.size();
				collData = fetchCdcNumber(searchCriteria, lReadCounter);
			}

			//Add the remaining CDC Numbers here.
			if (iInParameterCounter > 0) {
				aCdcNumbers[iCdcNumArrayIndex] = sbCdcNumbers.toString().substring(1);
			}
		}
	...
	@SuppressWarnings("unchecked")
	public Collection<PropertyContainer> fetchCdcNumber(SearchCriteriaInfo searchCriteria, long iStartIndex) throws DataException{
		Collection<PropertyContainer> collData = null;
			String sQueryString = getQueryFromSearchCriteria(searchCriteria, iStartIndex);
			ENEQueryResults eqRs = null;
			try {
